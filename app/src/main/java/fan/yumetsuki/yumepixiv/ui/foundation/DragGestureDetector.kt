package fan.yumetsuki.yumepixiv.ui.foundation
...
/**
 * requireUnconsumed 设置为 false，检测 drag 手势，不限制是否被消费，也不会消费 drag
 */
suspend fun PointerInputScope.detectVerticalDragGesturesIgnoreConsumed(
    onDragStart: (Offset) -> Unit = { },
    onDragEnd: () -> Unit = { },
    onDragCancel: () -> Unit = { },
    canDetectDrag: () -> Boolean = { true },
...
suspend fun AwaitPointerEventScope.drag(
    ...
    while (true) {
        val change = awaitDragOrCancellation(pointer, requireUnconsumed = requireUnconsumed) ?: return false
...
suspend fun AwaitPointerEventScope.awaitDragOrCancellation(
    ...
    return if (change?.isConsumed == false || !requireUnconsumed) change else null
}
    ...
    forEachGesture {
        awaitPointerEventScope {
            ...
            while (true) {
                ...
                if (drag == null || (drag.isConsumed && requireUnconsumed)) {
                    break
                }
                onDragStart.invoke(drag.position)
                onDrag(drag, overSlop)
                if (
                    !drag(drag.id, requireUnconsumed = false) {
                        onDrag(it, if (requireUnconsumed) it.positionChange() else it.positionChangeIgnoreConsumed())
                        if (requireUnconsumed) {
                            it.consume()
                        }
                    }
...
suspend fun PointerInputScope.detectVerticalDragGestures(
    ...
    forEachGesture {
        awaitPointerEventScope {
            ...
            var overSlop = 0f
            val drag = awaitVerticalPointerSlopOrCancellation(down.id, down.type, requireUnconsumed = requireUnconsumed) { change, over ->
                if (requireUnconsumed) {
                    change.consume()
                }
                overSlop = over
            }
            println("TestDelta: awaitSlop $drag")
            if (drag != null) {
                onDragStart.invoke(drag.position)
                onVerticalDrag.invoke(drag, overSlop)
                if (
                    verticalDrag(drag.id, requireUnconsumed = requireUnconsumed) {
                        println("TestDelta: onVerticalDrag $drag")
                        onVerticalDrag(it, if (requireUnconsumed) it.positionChange().y else it.positionChangeIgnoreConsumed().y)
                        if (requireUnconsumed) {
                            it.consume()
                        }
                    }
    ...
    forEachGesture {
        awaitPointerEventScope {
            ...
            if (drag != null) {
                ...
                if (
                    horizontalDrag(drag.id, requireUnconsumed = requireUnconsumed) {
                        onHorizontalDrag(it, if (requireUnconsumed) it.positionChange().x else it.positionChangeIgnoreConsumed().x)
                        if (requireUnconsumed) {
                            it.consume()
                        }
                    }
...
private suspend inline fun AwaitPointerEventScope.drag(
    ...
    while (true) {
        ...
        if (motionConsumed(change) && requireUnconsumed) {
            return false
        }
    ...
    while (true) {
        ...
        if (dragEvent.isConsumed && requireUnconsumed) {
            ...
            if (inDirection < touchSlop) {
                ...
                // 只要 requireUnconsumed 标识为 false，整个 drag 检测不消费任何手势
                if (!requireUnconsumed) {
                    return dragEvent
                }
